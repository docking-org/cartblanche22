...
class Item(db.Model):
    ...
    vendors = db.relationship('Vendor', backref='item', lazy='dynamic')

    def __repr__(self):
        return f'<Item {self.identifier} {self.quantity}{self.unit} from {self.database} database...Vendors are>'
    ...
    def getItem(self):
        ...
        for v in self.vendors:
            data.append(v.getVendor())
        dataa = {'identifier':self.identifier, 'quantity':self.quantity, 
                'unit':self.unit, 'compound_img':self.compound_img, 
                'database':self.database, 'vendors':data
                }
...
class Vendor(db.Model):
    ...
    def __repr__(self):
        return f'<{self.company_name} sells {self.quantity}{self.unit} for {self.price}$>'

    def createVendor(data):
        vendors = findVendors(data['id'])
        for v in vendors:
            vendor = Vendor(item_id = data['item_id'], supplier_code = v['supplier_code'],
                            cat_id_fk = v['cat_id_fk'], company_name = v['cat_name'],
    ...
    def getVendor(self):
        data = {'company_name':self.company_name, 'price':self.price, 'selected':self.selected}
        return json.dumps(data)
